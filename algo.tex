...
\State Return
\EndIf
\State Set Parent to itself
\If{Weakly Supported State}
\State Increment PC
...
\Else
\State Send Phantom Done message back to Sender
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}	
...
\ElsIf {Originator \& Recovering State} 
...
\If {Phantom Live State}
\State Send Build message to children or recovery return to parent if no children.
\ElsIf {Recovering state and $\neq$ originator}
\State Send Recovery return message to parent
\ElsIf {Forced Delete State and Originator}
